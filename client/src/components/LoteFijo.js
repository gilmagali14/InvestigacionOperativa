...
const LoteFijo = ({ articulos }) => {
    ...
    const calcularLoteOptimo = async (articulo) => {
        try {
            ...
            setLotesOptimos({ ...lotesOptimos, [articulo.idArticulo]: response.data });
        } catch (error) {
    ...
    const calcularPuntoPedido = async (articulo) => {
        try {
            ...
            setPuntosPedidos({ ...puntosPedidos, [articulo.idArticulo]: response.data });
        } catch (error) {
    ...
    const calcularCgi = async (articulo) => {
        try {
            ...
            setCgi({ ...cgi, [articulo.idArticulo]: response.data });
        } catch (error) {
                        ...
                        <tbody>
                            {articulos.map((articulo) => (
                                <tr key={articulo.idArticulo}>
                                    ...
                                    <td>
                                        {lotesOptimos[articulo.idArticulo] ? (
                                            <span>{lotesOptimos[articulo.idArticulo]}</span>
                                        ) : (
                                            <button
                                                className="btn btn-success"
                                                onClick={() => calcularLoteOptimo(articulo)}
                                                disabled={lotesOptimos[articulo.idArticulo]}
                                            >
                                    ...
                                    <td>
                                        {puntosPedidos[articulo.idArticulo] ? (
                                            <span>{puntosPedidos[articulo.idArticulo]}</span>
                                        ) : (
                                            <button
                                                className="btn btn-success"
                                                onClick={() => calcularPuntoPedido(articulo)}
                                                disabled={puntosPedidos[articulo.idArticulo]}
                                            >
                                    ...
                                    <td>
                                        {cgi[articulo.idArticulo] ? (
                                            <span>{cgi[articulo.idArticulo]}</span>
                                        ) : (
                                            <button
                                                className="btn btn-success"
                                                onClick={() => calcularCgi(articulo)}
                                                disabled={cgi[articulo.idArticulo]}
                                            >
                                                Calcular CGI
                                            </button>
                                        )}
